...
moveRightButton.addEventListener("touchstart", function () {
  platform.style.left =
    Math.min(
      gameArea.offsetWidth - platform.offsetWidth,
      platform.offsetLeft + 50
    ) + "px";
});

function resetObject() {
  var gameAreaWidth = gameArea.offsetWidth;
  var newLeft = Math.floor(
    Math.random() * (gameAreaWidth - object.offsetWidth)
  );
  ...
  fallingInterval = setInterval(function () {
    ...
    if (object.offsetTop + object.offsetHeight > gameArea.offsetHeight) {
      clearInterval(fallingInterval);
      if (
        object.offsetLeft < platform.offsetLeft + platform.offsetWidth &&
